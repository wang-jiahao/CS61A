...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    strategy0 = strategy(0, 0)
    for i in range(0, goal):
        for j in range(0, goal):
            if strategy(i, j) != strategy0:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max_rolls = 1
    score1 = averaged_dice(1, dice)
    for i in range(2, 11):
        scorei = averaged_dice(i, dice)
        if scorei > score1:
            max_rolls = i
            score1 = scorei
    return max_rolls
